...
export const mainFunc: any = async () => {
  ...
  try {
    ...
    await exec(`sqlite3 ${dbPath} "ALTER TABLE ${tableName} ADD Status TEXT" "UPDATE ${tableName} SET Status='draft'"`);

    const { stdout } = await exec(`sqlite3 ${dbPath} "SELECT COUNT(*) FROM ${tableName} WHERE Status='draft'"`);
    logger.log(`${tableName} now contains ${stdout.trim()} records having Status=draft`);

    logger.log(`Run command to see table schema: sqlite3 ${dbPath} ".schema ${tableName}"`);
  } catch (ex) {
    logger.error(ex);
  }
};
